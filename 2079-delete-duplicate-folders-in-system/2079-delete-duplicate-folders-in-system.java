class Solution {
  ...
  public List<List<String>> deleteDuplicateFolder(List<List<String>> paths) {
    ...
    Map<String, Integer> freq = new HashMap<>(); // hash table records the occurrence times of each serialized
                                                 // representation
    // post-order traversal based on depth-first search, calculate the serialized
    // representation of each node structure
    construct(root, freq);
  ...
  private void operate(
    ...
    if (freq.getOrDefault(node.serial, 0) > 1)
      return; // if the serialization representation appears more than once, it needs to be
              // deleted
